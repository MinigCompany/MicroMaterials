describe('Material API Tests', () => {
    ...
    it('Crear nueva salida para el material(Test 1)', () => {
      const newSalida = {
          material_ID: id._id,
          salida:{
              fecha: new Date().toISOString(),
    ...
    it('Crear nueva salida para el material(Test 2)', () => {
      const newSalida = {
          material_ID: id._id,
          salida:{
              fecha: new Date().toISOString(),
              nombreTrabajador:"Juan Test",
              cantidad:6,
              observacion:"Test Detail"
          }
      };
    ...
    it('Lista de entradas del material(Test)', () => {
      cy.request('GET', 'http://localhost:3000/ApiMinig/Materiales/AllEntradas/'+id._id)
        .then((response) => {
          expect(response.status).to.eq(200);
          expect(response.body).to.have.property('status',true);
      });
    });
    it('lista de Salidas del material(Test)', () => {
      const objeto = {
